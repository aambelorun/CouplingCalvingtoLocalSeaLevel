...
org=organizer('repository','Models','prefix', sprintf('cfSlr_%d_THICKNESS',sigma),'steps',steps);

% {{{ Mesh:
if perform(org,'2_Mesh'),
...
if perform(org, sprintf('%d_Loads', k)),  
    ...
    mdi = loadmodel(sprintf('../icesheetmodel/ModelsSeaTest/cfSlr_%d_fromobs', sigma)); %initial 1 year simulation
	mds = loadmodel(sprintf('../icesheetmodel/ModelsSeaTest/cfSlr_%d_fromdh%d', sigma, k-1)); %2nd 1 year simulation
...
if perform(org,sprintf('%d_Transient', k)),
	...
	md.miscellaneous.name=sprintf('cfSlr_%d_THICKNESS%d_Transient', sigma, k);
