...
if ~exist('k', 'var') || isempty(k)
        error('k not provided from bash');
end
...
org=organizer('repository','Models','prefix', sprintf('fullSlr_%d_THICKNESS',sigma), 'steps',steps);

% {{{ Mesh:
if perform(org,'_Mesh'),
...
% {{{Loads:
if perform(org, sprintf('%d_Loads', k)),  
	disp('   Step 2: Define loads in meters of ice height equivalent');
	md = loadmodel(org, '1_Mesh');
	mdi = loadmodel(sprintf('../icesheetmodel/ModelsSeaTest/fullSlr_%d_fromobs', sigma)); %initial 1 year simulation
    mds = loadmodel(sprintf('../icesheetmodel/ModelsSeaTest/fullSlr_%d_fromdh%d', sigma, k-1)); %2nd 1 year simulation

	[x_global, y_global]= ll2xy(md.mesh.lat, md.mesh.long, -1);

	h0 = mdi.geometry.thickness; % Initial thickness
...
% {{{ Parameterization:
if perform(org, sprintf('%d_Parameterization', k)),  
	disp('   Step 3: Parameterization');
	md = loadmodel(org, sprintf('%d_Loads', k));
...
% {{{ Transient:
if perform(org,sprintf('%d_Transient', k)),
	disp('   Step 4: Transient run');
	md = loadmodel(org, sprintf('%d_Parameterization', k));
	mdi = loadmodel(sprintf('../icesheetmodel/ModelsSeaTest/fullSlr_%d_fromobs', sigma)); %initial 1 year simulation
    mds = loadmodel(sprintf('../icesheetmodel/ModelsSeaTest/fullSlr_%d_fromdh%d', sigma, k-1)); %2nd 1 year simulation
	...
	md.miscellaneous.name=sprintf('fullSlr_%d_THICKNESS%d_Transient', sigma, k);
