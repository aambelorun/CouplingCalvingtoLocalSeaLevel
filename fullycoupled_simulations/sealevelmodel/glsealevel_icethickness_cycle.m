...
org=organizer('repository','Models','prefix', sprintf('glSlr_%d_THICKNESS',sigma),'steps',steps);
...
if perform(org, sprintf('%d_Loads', k)),  
    ...
    mdi = loadmodel(sprintf('../icesheetmodel/ModelsSeaTest/glSlr_%d_fromobs', sigma)); %initial 1 year simulation
	mds = loadmodel(sprintf('../icesheetmodel/ModelsSeaTest/glSlr_%d_fromdh%d', sigma, k-1)); %2nd 1 year simulation
...
if perform(org,sprintf('%d_Transient', k)),
	...
	md.miscellaneous.name=sprintf('glSlr_%d_THICKNESS%d_Transient', sigma, k);
