...
org=organizer('repository','ModelsSeaTest','prefix', sprintf('glSlr_%d_', sigma),'steps',steps);

addpath('../../Functions');

% {{{ SealevelVMcalvingtest_realisticsealevel

if perform(org,'fromobs')

    mds = loadmodel('ModelsSeaTest/SlrGRACE+ICESAT_Transient.mat');
    md = loadmodel('../../initialization/Models/Amundsen_Transient.mat');
    ...
    md.miscellaneous.name=sprintf('glSlr_%d_fromobs', sigma);
    md.settings.waitonlock = 0;
    md.verbose=verbose('solution',true,'convergence',true);
    md.cluster = mycluster;
    md.transient.requested_outputs={'default','IceVolume','IceVolumeAboveFloatation'}; %For rate-based calving laws
...
% {{{
if perform(org,'fromdh1')

    mds = loadmodel(sprintf('../sealevelmodel/Models/glSlr_%d_THICKNESS1_Transient.mat', sigma));
    md = loadmodel(org,'fromobs');
    ...
    md.miscellaneous.name=sprintf('glSlr_%d_fromdh1', sigma);
    md.settings.waitonlock = 0;
    md.verbose=verbose('solution',true,'convergence',true);
    md.cluster = mycluster;
    md.transient.requested_outputs={'default','IceVolume','IceVolumeAboveFloatation'}; %For rate-based calving laws
