...
org=organizer('repository','ModelsSeaTest','prefix', sprintf('cfSlr_%d_',sigma),'steps',steps);
...
if perform(org,'fromobs')
	...
	md.settings.output_frequency=1;
   	...
   	md.miscellaneous.name=sprintf('cfSlr_%d_fromobs', sigma);
...
if perform(org,'fromdh1')
    ...
    mds = loadmodel(sprintf('../sealevelmodel/Models/cfSlr_%d_THICKNESS1_Transient.mat', sigma));
    ...
    md.miscellaneous.name=sprintf('cfSlr_%d_fromdh1', sigma);
    md.settings.waitonlock = 0;
    md.verbose=verbose('solution',true,'convergence',true);
	md.cluster = mycluster;
    md.transient.requested_outputs={'default','IceVolume','IceVolumeAboveFloatation'};
